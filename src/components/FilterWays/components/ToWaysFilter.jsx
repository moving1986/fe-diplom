...
const ToWayFilter = () => {
    ...
    // Исправленная инициализация состояний для обратного пути
    const [departureFrom, setDepartureFrom] = useState(
        filter.end_departure_hour_from !== undefined && filter.end_departure_hour_from !== null 
            ? filter.end_departure_hour_from 
            : minHour
    );
    const [departureTo, setDepartureTo] = useState(
        filter.end_departure_hour_to !== undefined && filter.end_departure_hour_to !== null 
            ? filter.end_departure_hour_to 
            : maxHour
    );
    const [arrivalFrom, setArrivalFrom] = useState(
        filter.end_arrival_hour_from !== undefined && filter.end_arrival_hour_from !== null 
            ? filter.end_arrival_hour_from 
            : minHour
    );
    const [arrivalTo, setArrivalTo] = useState(
        filter.end_arrival_hour_to !== undefined && filter.end_arrival_hour_to !== null 
            ? filter.end_arrival_hour_to 
            : maxHour
    );
    ...
    useEffect(() => {
        ...
        setDepartureFrom(validateValue(filter.end_departure_hour_from, minHour));
        setDepartureTo(validateValue(filter.end_departure_hour_to, maxHour));
        setArrivalFrom(validateValue(filter.end_arrival_hour_from, minHour));
        setArrivalTo(validateValue(filter.end_arrival_hour_to, maxHour));
    }, [
        filter.end_departure_hour_from, 
        filter.end_departure_hour_to,
        filter.end_arrival_hour_from,
    ...
    const handleMouseUp = (type) => {
        switch (type) {
            case 'departure_from':
                dispatch(changeTripsSearchInput({ name: 'end_departure_hour_from', value: departureFrom }));
                break;
            case 'departure_to':
                dispatch(changeTripsSearchInput({ name: 'end_departure_hour_to', value: departureTo }));
                break;
            case 'arrival_from':
                dispatch(changeTripsSearchInput({ name: 'end_arrival_hour_from', value: arrivalFrom }));
                break;
            case 'arrival_to':
                dispatch(changeTripsSearchInput({ name: 'end_arrival_hour_to', value: arrivalTo }));
                break;
            default:
                break;
        }
